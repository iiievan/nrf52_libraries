...
#define CASE(str)    static_assert(switch_string::str_is_correct(str) && (switch_string::str_len(str) <= switch_string::MAX_STRING_LEN),\
...
{
    const unsigned char MAX_STRING_LEN = 9;
    ...
    {
        return (str_is_correct(str) && (str_len(str) <= MAX_STRING_LEN)) ? str_hash(str, str_len(str)) : N_HASH;
    }

    //inline unsigned long long str_hash_for_switch(const std::string& str)
    //{
    //    return (str_is_correct(str.c_str()) && (str.length() <= MAX_STRING_LEN)) ? str_hash(str.c_str(), str.length()) : N_HASH;
    //}
}
